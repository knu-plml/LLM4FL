...
def fine_tune(training_file, validation_file, epochs, batch_size, save_dir, parallel=False, load_range=None):
    ...
    if not parallel:
        ...
        model = PLBartForConditionalGeneration.from_pretrained(pretrained_file, device_map="auto")
...
if __name__ == '__main__':
    model_name = 'plbart-base' # ['plbart-base', 'plbart-large']
    iterN = '0'
    ...
    fine_tune(
        training_file, validation_file, epochs=1, batch_size=batch_size, save_dir='../../models/Java-finetuned-model/'+model_name+'-finetune/' + iterN + '/', parallel=False, load_range=None
    )

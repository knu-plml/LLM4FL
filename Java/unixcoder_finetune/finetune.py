...
def fine_tune(training_file, validation_file, epochs, batch_size, save_dir, parallel=False, load_range=None):
    ...
    training_dataset = Dataset(training_file, tokenizer, max_length=989, shuffle=False, load_range=load_range)
    validation_dataset = Dataset(validation_file, tokenizer, max_length=989, load_range=load_range)
...
if __name__ == '__main__':
    ...
    fine_tune(
        training_file, validation_file, epochs=1, batch_size=10, save_dir='../../models/Java-finetuned-model/'+model_name+'-finetune/' + iterN + '/', parallel=False, load_range=None
    )

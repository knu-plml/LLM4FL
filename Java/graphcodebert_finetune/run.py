...
def main():
    ...
    if args.do_test:
        for j in range(5):
            graphcodebert_output = json.load(open(args.test_filename, 'r'))
            graphcodebert_output['model'] = 'graphcodebert-base'
            model.load_state_dict(torch.load(args.load_model_path + '/' + str(j) + '/pytorch_model.bin'))
            model.to(device_id)
            model.eval() 
            for i, filename in enumerate(graphcodebert_output['data']):
                print(i + 1, 'generating', filename)
                eval_example = read_example(graphcodebert_output['data'][filename])
                ...
                for batch in eval_dataloader:
                    ...
                    graphcodebert_output['data'][filename]['output'] = output
                    graphcodebert_output['data'][filename]['input'] = eval_example.source
                    graphcodebert_output['data'][filename]['gold_output'] = eval_example.target

            if not os.path.exists(args.output_dir):
                os.makedirs(args.output_dir)
            json.dump(graphcodebert_output, open(args.output_dir + '/graphcodebert_base_output' + str(j) + '.json', 'w'), indent=3)

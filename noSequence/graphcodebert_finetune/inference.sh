...
test_model=../../models/noSequence-finetuned-model/graphcodebert-base-finetune/

mkdir -p $output_dir

python run.py --do_test --model_type roberta --model_name_or_path $pretrained_model --tokenizer_name $pretrained_model --config_name $pretrained_model --load_model_path $test_model --test_filename $test_file --output_dir $output_dir --max_source_length $source_length --max_target_length $target_length --beam_size $beam_size --eval_batch_size $batch_size

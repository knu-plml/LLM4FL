...
def main():
    ...
    if args.do_train:
        ...
        examples = read_examples(args.train_filename)
        examLen = int(len(examples))
        training_dataset = examples[0:int(examLen * args.seed * 0.2)] + examples[int(examLen * (args.seed + 1 * 0.2):]

        train_features = convert_examples_to_features(training_dataset, tokenizer,args,stage='train')
        ...
        logger.info("  Num examples = %d", len(training_dataset))
        logger.info("  Batch size = %d", args.train_batch_size)
        logger.info("  Num epoch = %d", num_train_optimization_steps*args.train_batch_size//len(training_dataset))
    ...
    if args.do_test:
        examples = read_examples(args.test_filename)
        for j in range(5):
            model.load_state_dict(torch.load(args.load_model_path + '/' + str(j) + '/pytorch_model.bin'))
            model.to(device_id)
            model.eval() 

            eval_examples = examples[int(len(examples) * (j * 0.2 + 0.1)):int(len(examples) * (j + 1) * 0.2)]
            codebert_output = {'config': 'finetune', 'data': {}, 'model': 'codebert'}
            for i, eval_example in enumerate(eval_examples):
                ...
                codebert_output['data'][str(i)] = {}
                codebert_output['data'][str(i)]['output'] = output
                codebert_output['data'][str(i)]['input'] = eval_example.source
                codebert_output['data'][str(i)]['gold_output'] = eval_example.target

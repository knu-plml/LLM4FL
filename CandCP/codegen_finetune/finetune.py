...
def fine_tune(training_file, epochs, batch_size, save_dir, parallel=False, load_range=None):
    ...
    if not parallel:
        ...
        model = CodeGenForCausalLM.from_pretrained(pretrained_file, device_map="auto")
    ...
    dataset = Dataset(training_file, tokenizer, max_length=992, shuffle=False, load_range=load_range)
...
if __name__ == '__main__':
    model_name = 'codegen-350M' # codegen-350M codegen-2B codegen-6B
    iterN = '0'
    ...
    training_file = '../../data/CVEfixesCandCP.jsonl'      # change to fine-tuning data path
    ...
    fine_tune(
        training_file, epochs=1, batch_size=batch_size, save_dir='../../models/CandCP-finetuned-model/'+model_name+'-finetune/' + iterN + '/', parallel=parallel, load_range=None
    )

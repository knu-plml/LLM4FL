...
def incoder_finetune_output(input_file, output_file, model_dir, model_name, index, num_output=10):
  tokenizer = AutoTokenizer.from_pretrained('/'.join(model_dir.split('/')[:-2]) + '/' + model_name[:-9])
  model = AutoModelForCausalLM.from_pretrained(model_dir + model_name + '/' + str(index), device_map='auto')
...
if __name__ == '__main__':
  ...
  model_names = ['incoder-6B-finetune', 'incoder-1B-finetune']
  ...
  for model_name in model_names:
    ...
    output_file = './result/' + model_name + '_output.json'

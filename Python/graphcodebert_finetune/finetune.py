    ...
    with torch.no_grad():
        for i, data in enumerate(validation_loader):
            data = {
                ...
                'pos_idx': data['pos_idx'].to(device_ids[0]),
                'input_mask': data['input_mask'].to(device_ids[0]),
                'labels_mask': data['labels_mask'].to(device_ids[0]),
                'attention_mask': data['attention_mask'].to(device_ids[0]),
            }
            output = model(data['input_ids'], data['input_mask'], data['pos_idx'], data['attention_mask'], data['labels'],  target_mask=data['labels_mask'])
            loss = output[0]
...
def fine_tune(training_file, epochs, batch_size, save_dir, parallel=False, load_range=None):
    ...
    encoder = RobertaModel.from_pretrained(pretrained_file)
    ...
    dataset = Dataset(training_file, tokenizer, max_length=480, shuffle=False, load_range=[0,120000])
...
if __name__ == '__main__':
    ...
    training_file = '../../data/codeNetPython.jsonl'
    ...
    fine_tune(
        training_file, epochs=1, batch_size=20, save_dir='../../models/Python-finetuned-model/'+model_name+'-finetune/' + iterN + '/', parallel=False, load_range=None
    )

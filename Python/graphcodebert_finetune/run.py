...
def read_examples(filename):
    ...
    examples=[]
    dataList = json.load(open(filename))
...
def main():
    ...
    if args.do_train:
        ...
        examples = read_examples(args.train_filename)[:120000]
        ...
        for epoch in range(args.num_train_epochs):
            ...
            #output_model_file = os.path.join(last_output_dir, "pytorch_model_" + str(epoch) + ".bin")
            output_model_file = os.path.join(last_output_dir, "pytorch_model" + ".bin")
            torch.save(model_to_save.state_dict(), output_model_file)                    
               
    if args.do_test:
        examples = read_examples(args.test_filename)[:120000]
        eval_examples = examples[int(len(examples) * 0.9):]
        for j in range(5):
            model.load_state_dict(torch.load(args.load_model_path  + '/pytorch_model.bin'))
            model.to(device_id)
            model.eval() 
            
            graphcodebert_output = {'config': 'finetune', 'data': {}, 'model': 'graphcodebert'}
            for i, eval_example in enumerate(eval_examples):
                print(i + 1, 'generating...')
                eval_feature = convert_examples_to_features([eval_example], tokenizer, args, stage='test')
                eval_data = TextDataset(eval_feature, args)
                eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size, num_workers=4)
                for batch in eval_dataloader:
                    batch = tuple(t.to(device_id) for t in batch)
                    source_id, source_mask, position_idx, att_mask, target_id, target_mask = batch
                    if source_id.size(1) >= 768:
                        print('too long:', input_ids.size(1))
                        continue
                    with torch.no_grad():
                        try:
                            generated_ids = model(source_id, source_mask, position_idx, att_mask)
                    ...
                    graphcodebert_output['data'][str(i)] = {}
                    graphcodebert_output['data'][str(i)]['output'] = output
                    graphcodebert_output['data'][str(i)]['input'] = eval_example.source
                    graphcodebert_output['data'][str(i)]['gold_output'] = eval_example.target
            if not os.path.exists(args.output_dir):
                os.makedirs(args.output_dir)
            json.dump(graphcodebert_output, open(args.output_dir + '/graphcodebert_output' + str(j) + '.json', 'w'), indent=2)

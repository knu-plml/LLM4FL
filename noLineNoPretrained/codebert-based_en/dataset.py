    ...
    def __init__(self, file_path, tokenizer, max_length=512, shuffle=False, load_range=None):
        ...
        for fileNum, l in enumerate(fp.readlines()):
            ...
            while True: 
                ...
                currInputs = [tokenizer.bos_token] + tokenizer.tokenize('\n'.join(list(map(lambda x: '\t'.join(x.split('\t')[1:]), inputs[idx:idx+interval])))) + [tokenizer.sep_token]
                ...
                for fIdx, f in enumerate(faultLocs):
                    if idx + 1 <= f and f <= idx + 1 + interval:
                        currOutputs.append('\t'.join(outputs[fIdx].split('\t')[1:]))

                if currOutputs == []:
                    currOutputs = noFaultOutputs

                currOutputs = [tokenizer.bos_token] + tokenizer.tokenize('\n'.join(currOutputs)) + [tokenizer.sep_token]
                currOutputs = torch.tensor([tokenizer.convert_tokens_to_ids(currOutputs)])

                idx += step
                step = interval - step

                if currInputs.size(1) > max_length + 2 or currOutputs.size(1) > 34:
                    count += 1
                    continue
                ...
                self.data.append({
                    ...
                    'labels_mask': torch.ones(currOutputs.size()).long()
                })
...
def custom_collate(batch):
    batch_data = {'input_ids': [], 'labels': [], 'attention_mask': [], 'labels_mask': []}
    ...
    for b in batch:
        ...
        batch_data['labels_mask'].append(torch.cat([b['labels_mask'], torch.zeros(1, max_output_len - b['labels_mask'].size(1))], dim=1))

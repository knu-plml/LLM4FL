...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, file_path, tokenizer, max_length=768, shuffle=False, load_range=None):
        self.data = []
        ...
        for fileNum, l in enumerate(fp.readlines()):
            ...
            #print(inputs)                                                                                                                
            #print(tokenizer.encode('\n'.join(inputs) + tokenizer.eos_token))
            #print()
            #print(tokenizer.convert_tokens_to_ids([tokenizer.bos_token] + tokenizer.tokenize('\n'.join(inputs)) + [tokenizer.eos_token]))
            #print()                                                 
            #exit()
            ...
            while True:
                ...
                currInputs = tokenizer.tokenize('\n'.join(list(map(lambda x: '\t'.join(x.split('\t')[1:]), inputs[idx:idx+interval])))) # + [tokenizer.eos_token]
                ...
                currOutputs = tokenizer.tokenize('\n'.join(currOutputs)) + [tokenizer.eos_token]
